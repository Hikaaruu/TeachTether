...
{
    public class CanCreateAnnouncementHandler : AuthorizationHandler<CanCreateAnnouncementRequirement, List<int>>
    {
        private readonly IUnitOfWork _unitOfWork;

        public CanCreateAnnouncementHandler(IUnitOfWork unitOfWork)
        {
            _unitOfWork = unitOfWork;
        }

        protected override async Task HandleRequirementAsync(AuthorizationHandlerContext context, CanCreateAnnouncementRequirement requirement, List<int> classGroupIds)
        {
            ...
            foreach (var cgId in classGroupIds)
            {
                ...
                if (!(isAssignedToClassGroup || isHomeTeacher))
                {
                    return;
                }
            }
