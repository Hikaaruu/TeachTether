...
public class ClassGroupsSubjectDeletionHelper(IUnitOfWork unitOfWork) : IClassGroupsSubjectDeletionHelper
{
    ...
    public async Task DeleteClassGroupsSubjectAsync(int id)
    {
        ...
        var grades =
            await _unitOfWork.StudentGrades.GetAllAsync(g =>
                g.SubjectId == subjectId && studentIds.Contains(g.StudentId));
        _unitOfWork.StudentGrades.DeleteMany(grades);

        var behavior =
            await _unitOfWork.StudentBehaviors.GetAllAsync(g =>
                g.SubjectId == subjectId && studentIds.Contains(g.StudentId));
        _unitOfWork.StudentBehaviors.DeleteMany(behavior);

        var att = await _unitOfWork.StudentAttendances.GetAllAsync(g =>
            g.SubjectId == subjectId && studentIds.Contains(g.StudentId));
