...
public class StudentAttendanceService(IUnitOfWork unitOfWork, IMapper mapper, IUserService userService)
    : IStudentAttendanceService
{
    ...
    public async Task<StudentAttendanceResponse> CreateAsync(CreateStudentAttendanceRequest request, int teacherId,
        int studentId)
    {
    ...
    public async Task<IEnumerable<StudentAttendanceResponse>> GetAllByStudentAsync(int studentId, int subjectId)
    {
        ...
        var teachers =
            await _unitOfWork.Teachers.GetByIdsAsync(studentAttendances.Select(sg => sg.TeacherId).Distinct());
