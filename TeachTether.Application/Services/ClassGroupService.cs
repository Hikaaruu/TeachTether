...
public class ClassGroupService(IUnitOfWork unitOfWork, IMapper mapper, IClassGroupDeletionHelper deletionHelper)
    : IClassGroupService
{
    private readonly IClassGroupDeletionHelper _deletionHelper = deletionHelper;
    ...
    public async Task<ClassGroupResponse> CreateAsync(CreateClassGroupRequest request, int schoolId)
    {
        ...
        if (await _unitOfWork.ClassGroups.AnyAsync(cg =>
            ...
            throw new BadRequestException(
                $"Class group {request.GradeYear} - {request.Section} already exists in school {schoolId}.");
    ...
    public async Task<IEnumerable<ClassGroupResponse>> GetAvailableForTeacherAsync(int teacherId)
    {
        ...
        var classGroupIds =
            (await _unitOfWork.ClassGroupsSubjects.GetByIdsAsync(clsGrpSubjIds)).Select(cgs => cgs.ClassGroupId);
    ...
    public async Task UpdateAsync(int id, UpdateClassGroupRequest request)
    {
        ...
        if ((classGroup.Section != request.Section || classGroup.GradeYear != request.GradeYear) &&
            await _unitOfWork.ClassGroups.AnyAsync(cg =>
                ...
                cg.Section == request.Section))
            throw new BadRequestException(
                $"Class group {request.GradeYear} - {request.Section} already exists in school {classGroup.SchoolId}.");
