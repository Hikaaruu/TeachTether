...
namespace TeachTether.Application.Services
{
    public class AnalyticsService : IAnalyticsService
    {
        ...
        public async Task<ClassAveragesResponse> GetClassAverages(int subjectId, int studentId, int classGroupId)
        {
            ...
            {
                attendance.PresentPercentage = attendances.Count(a => a.Status == AttendanceStatus.Present) * 100m / totalAttendance;
                attendance.LatePercentage = attendances.Count(a => a.Status == AttendanceStatus.Late) * 100m / totalAttendance;
                attendance.AbsentPercentage = attendances.Count(a => a.Status == AttendanceStatus.Absent) * 100m / totalAttendance;
                attendance.ExcusedPercentage = attendances.Count(a => a.Status == AttendanceStatus.Excused) * 100m / totalAttendance;
            }
